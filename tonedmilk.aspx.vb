...
Partial Class tonedmilk
    ...
    Protected Sub btnbuy_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnbuy.Click
        ...
        lbltotal.Text = Val(lblt1.Text) + Val(lblt2.Text) + Val(lblt3.Text) + Val(lblt4.Text)

        da = New SqlDataAdapter("insert into toned_milk(name,mnumber,address,pincode,city,state,size_180ml,size_500ml,size_1L,size_6L,total) values('" & txtname.Text & "','" & txtmno.Text & "','" & txtaddress.Text & "','" & txtpincode.Text & "','" & txtcity.Text & "','" & txtstate.Text & "','" & txtq1.Text & "','" & txtq2.Text & "','" & txtq3.Text & "','" & txtq4.Text & "','" & lbltotal.Text & "')", cn)
        ds = New DataSet
        da.Fill(ds)
    ...
    Protected Sub btnserchorder_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnserchorder.Click
        da = New SqlDataAdapter("select * from toned_milk where name='" & txtname.Text & "'", cn)
        ds = New DataSet
        da.Fill(ds)
        ...
        lbltotal.Text = ds.Tables(0).Rows(0).Item(10).ToString
    ...
    Protected Sub btnupdateorder_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnupdateorder.Click
        ...
        lbltotal.Text = Val(lblt1.Text) + Val(lblt2.Text) + Val(lblt3.Text) + Val(lblt4.Text)
        da = New SqlDataAdapter("update toned_milk set name='" & txtname.Text & "',mnumber='" & txtmno.Text & "',address='" & txtaddress.Text & "',pincode='" & txtpincode.Text & "',city='" & txtcity.Text & "',state='" & txtstate.Text & "',size_180ml='" & txtq1.Text & "',size_500ml='" & txtq2.Text & "',size_1L='" & txtq3.Text & "',size_6L='" & txtq4.Text & "',total='" & lbltotal.Text & "' where name='" & txtname.Text & "'", cn)
        ds = New DataSet
        da.Fill(ds)
